...
export default React.memo(function FinanceSimulatorTab({
  leads,
  theme = "escuro",
  accSettings,
  addNotification,
  ...
  const [activeSim, setActiveSim] = useState<"financing" | "sales">(
    "financing",
  );

  // Common/Interactive Selected Lead State
  const [selectedLeadId, setSelectedLeadId] = useState<string>(
    isInline && leads.length > 0 ? leads[0].id : "",
  );
  ...
  const filteredLeadsForLink = React.useMemo(() => {
    return leads.filter((lead) => {
      ...
      const matchesSearch = searchTerms.every((term) =>
        leadString.includes(term),
      );
      const matchesInitial =
        initialLetterFilter === "todos" ||
        lead.name.trim().charAt(0).toUpperCase() ===
          initialLetterFilter.toUpperCase();
  ...
  const leadsAtivos = filteredLeadsForLink.filter(
    (l) =>
      l.status !== "novo" &&
      l.status !== "arquivado" &&
      l.status !== "perdi" &&
      l.status !== "perdido" &&
      l.status !== "reprovado_credito",
  );
  const leadsArquivados = filteredLeadsForLink.filter(
    (l) =>
      l.status === "arquivado" ||
      l.status === "perdi" ||
      l.status === "perdido" ||
  ...
  const [simulatedPropertyPrice, setSimulatedPropertyPrice] =
    useState<number>(275000); // Property cost
  const [loanMonths, setLoanMonths] = useState<number>(360); // Amortization period in months (max 420)
  const [amortizationSystem, setAmortizationSystem] = useState<"SAC" | "PRICE">(
    "SAC",
  );

  // NOVOS PARAMETROS REQUISITADOS
  const [professionType, setProfessionType] = useState<"CLT" | "AUTONOMO">(
    "CLT",
  );
  ...
  const [clientCustomName, setClientCustomName] =
    useState<string>("Cliente Avulso");
  ...
  const [applicableInterestRate, setApplicableInterestRate] =
    useState<number>(0);
  ...
  useEffect(() => {
    const metrics = calculateAdvancedMetrics(
      simulatedPropertyPrice,
      simulatedPropertyPrice * 0.01, // minIncome estimation
      professionType === 'CLT' ? cltIncome : autonomoIncome,
      spouseIncome,
      isMarried,
      hasFGTS || hasInterestReducer,
      hasDependents,
      applicantAge,
      true, // hasCleanCredit: assume true for simulator if not specified
      amortizationSystem,
      0, // fgtsBalance not explicitly in simulator
      0 // ownSavings not explicitly in simulator
    );

    setCalculatedSubsidy(Math.round(metrics.subsidy));
    setApplicableInterestRate(metrics.annualRate);
    setMaxMonthlyPayment(Math.round(metrics.maxAllowedInstallment));
    setFinancedAmount(Math.round(metrics.approvedLoan));
    setFirstPayment(Math.round(metrics.initialInstallment));
    setLastPayment(Math.round(metrics.finalInstallment));
    setRequiredDownpayment(Math.round(metrics.totalDownPaymentRequired));
    setMcmvBracketName(grossIncome <= 3200 ? "Faixa 1 (MCMV)" : grossIncome <= 4400 ? "Faixa 2 (MCMV)" : grossIncome <= 8000 ? "Faixa 3 (MCMV)" : "SBPE");
  }, [
    grossIncome,
    applicantAge,
    hasDependents,
  ...
  const [salesConstructionMonths, setSalesConstructionMonths] =
    useState<number>(36);
  ...
  const [salesFinanciamentoPct, setSalesFinanciamentoPct] =
    useState<number>(65); // Bank financing post-construction
  ...
  const [salesLeftoverForMonthlies, setSalesLeftoverForMonthlies] =
    useState<number>(0);
  ...
  const applyPresetProfile = (presetKey: string) => {
    ...
    if (presetKey === "mcmv_padrao") {
      ...
      if (addNotification)
        addNotification(
          "📊 PRESORT MCMV",
          "Configurações de fluxo de obras para o Minha Casa Minha Vida aplicadas.",
          "success",
        );
      ...
      if (addNotification)
        addNotification(
          "📊 PRESET SBPE",
          "Configurações equilibradas de fluxo SBPE médio-padrão aplicadas.",
          "success",
        );
    } else {
      ...
      if (addNotification)
        addNotification(
          "📊 PRESET INVESTIDOR",
          "Amortizações agressivas para o investidor de liquidez do estoque.",
          "info",
        );
    }
  };
  ...
  const handleExportCSV1 = () => {
    ...
    const csvContent = [
      ...
      [
        "Regime de Comprovacao de Renda",
        professionType === "CLT"
          ? "💼 CLT (Holerite)"
          : "🚀 Autônomo (Extrato)",
        "Modo de analise de credito",
      ],
      [
        "Renda Proponente Principal",
        `R$ ${(professionType === "CLT" ? cltIncome : autonomoIncome).toLocaleString("pt-BR")}`,
        "Renda individual",
      ],
      [
        "Composicao Familiar (Casado)",
        isMarried
          ? `Sim (+ R$ ${spouseIncome.toLocaleString("pt-BR")} do conjuge)`
          : "Nao",
      ...
      [
        ...
        "Bonus MCMV aplicados",
      ],
      [
        "Renda Familiar Consolidada",
        `R$ ${grossIncome.toLocaleString("pt-BR")}`,
        "Renda familiar total",
      ],
      [
        "Enquadramento Habitacional",
        mcmvBracketName,
        "Faixa de qualificacao MCMV",
      ],
      [
        "Idade do Proponente",
        `${applicantAge} anos`,
        "Termo limitante de amortizacao",
      ],
      [
        "Tempo de FGTS (3+ anos)",
        hasFGTS ? "Ativo (Taxa de Juros Reduzida)" : "Inativo",
        "Regra de bonificacao de juros",
      ],
      [
        "Sistema de Amortizacao",
        amortizationSystem,
        "Tabela de amortizacao contratada (SAC ou PRICE)",
      ],
      [
        "Prazo Solicitado",
        `${loanMonths} meses`,
        "Tempo total de parcelas bancarias",
      ],
      [""],
      ["--- RESULTADOS E BENEFICIOS HABITACIONAIS ---"],
      [
        "Preco do Imovel Simulado",
        `R$ ${simulatedPropertyPrice.toLocaleString("pt-BR")}`,
        "Avaliacao da Unidade",
      ],
      [
        "Taxa Nominal de Juros",
        `${applicableInterestRate}% a.a.`,
        "Taxa de juros anualizada",
      ],
      [
        "Subsidio Habitacional (Desconto Caixa)",
        `R$ ${calculatedSubsidy.toLocaleString("pt-BR")}`,
        "Fator de desconto direto do Governo",
      ],
      [
        "Saldo Financiado Garantido Caixa",
        `R$ ${financedAmount.toLocaleString("pt-BR")}`,
        "Fundo Caixa a ser repassado",
      ],
      [
        "Primeira Prestacao Bancaria",
        `R$ ${firstPayment.toLocaleString("pt-BR")}`,
        "Mensalidade inicial corrigida",
      ],
      [
        "Ultima Prestacao Bancaria",
        `R$ ${lastPayment.toLocaleString("pt-BR")}`,
        "Prestacao final de quitacao",
      ],
      [
        "Capacidade Recomendada (30% Renda)",
        `R$ ${maxMonthlyPayment.toLocaleString("pt-BR")}`,
        "Margem de comprometimento maxima",
      ],
      [
        "Valor de Entrada Necessaria (Recursos Proprios)",
        `R$ ${requiredDownpayment.toLocaleString("pt-BR")}`,
        "Total devido a Construtora",
      ],
      [""],
      ["--- PLANO DE PARCELAMENTO FACILITADO (PERIODO DE OBRAS) ---"],
      [
        "Ato Real de Sinal de Entrada",
        `R$ ${valorAto.toLocaleString("pt-BR")}`,
        "Ato no fechamento do contrato",
      ],
      [
        "2x Parcelas Baloes Anuais (Cada)",
        `R$ ${valorAnual.toLocaleString("pt-BR")}`,
        `Totalizando R$ ${(2 * valorAnual).toLocaleString("pt-BR")}`,
      ],
      [
        "Parcela de Chaves (Entrega)",
        `R$ ${valorChaves.toLocaleString("pt-BR")}`,
        "Exigido para entrega de chaves",
      ],
      [
        "Saldo Residual para Parcelar na Obra",
        `R$ ${difRestante.toLocaleString("pt-BR")}`,
        "Diferenca a diluir nas parcelas de obra",
      ],
      [
        "Mensalidade no Periodo de Obras",
        `${tempoObra} parcelas de R$ ${Math.round(valMensalObra).toLocaleString("pt-BR")} /mes`,
        `Amortizado durante os ${tempoObra} meses em obras`,
      ],
      [""],
      ["--- DOSSIE DIGITAL DE DOCUMENTOS FISICOS ---"],
      [
        "Documento CPF / CNH com foto",
        docCpfCnh ? "CONFIRMADO [✓]" : "PENDENTE [ ]",
        "ID Fisica",
      ],
      [
        "Comprovante de Endereco (<90 dias)",
        docEndereco ? "CONFIRMADO [✓]" : "PENDENTE [ ]",
        "Contas basicas de consumo",
      ],
      [
        "Comprovante de Renda (Holerite/IPRF)",
        docRenda ? "CONFIRMADO [✓]" : "PENDENTE [ ]",
        "Demonstrativos oficiais de renda",
      ],
      [
        "Extratos Bancarios Consolidados",
        docExtratos ? "CONFIRMADO [✓]" : "PENDENTE [ ]",
        "Soma dos ultimos 3 meses com carimbos",
      ],
      [
        "Status Consolidado do Dossie",
        docCpfCnh && docEndereco && docRenda && docExtratos
          ? "COMPLETO (Pronto para CAIXA)"
          : `EM ANALISE (${[docCpfCnh, docEndereco, docRenda, docExtratos].filter(Boolean).length}/4)`,
        "Dossie fiscal do comprador",
      ],
    ]
      .map((row) => row.join(";"))
      .join("\n");
    ...
    downloadCSV(
      csvContent,
      `Spreadsheet_Financiamento_MCMV_${clientName.replace(/\s+/g, "_")}.csv`,
    );
    if (addNotification) {
      addNotification(
        "📊 PLANILHA EXPORTADA",
        "Planilha de enquadramento (CSV) gerada e baixada com sucesso!",
        "success",
      );
    }
  };
  ...
  const handleExportCSV2 = () => {
    ...
    let csvData = [
      ...
      [
        "Modulo",
        "Descritivo da Parcela",
        "Percentual (%)",
        "Valor Estimado (R$)",
      ],
      [
        "Sinal Prontidao",
        "Ato de recursos proprios na assinatura",
        `${salesEntradaPct}%`,
    ...
    csvData.push([
      "Valor do Imovel",
      `R$ ${salesPropertyPrice.toLocaleString("pt-BR")}`,
    ]);
    csvData.push([
      "Meses de Obras Disponivel",
      `${salesConstructionMonths} meses`,
    ]);
    csvData.push([
      "Status de Fechamento",
      isSalesBalanced
        ? "PLANILHA DUPLICADA 100%"
        : "PLANILHA REQUER EQUILIBRIO",
    ]);

    const csvContent = csvData.map((row) => row.join(";")).join("\n");
    downloadCSV(
      csvContent,
      `Roteiro_Fluxo_Obras_${clientName.replace(/\s+/g, "_")}.csv`,
    );

    if (addNotification) {
      addNotification(
        "📊 PLANILHA EXPORTADA",
        "Planilha de fluxo periódico (CSV) baixada com sucesso!",
        "success",
      );
    }
  };
...
`;
    ...
    if (addNotification) {
      addNotification(
        "📋 SIMULAÇÃO COPIADA",
        "Demonstrativo completo (Caixa + Entrada Facilitada Construtora) copiado!",
        "success",
      );
    }
  };
...
`;
    ...
    if (addNotification) {
      addNotification(
        "📋 PLANILHA COPIADA",
        "Planilha de fluxo de obras e disponível copiada para área de transferência!",
        "success",
      );
    }
  };

  const isSalesBalanced =
    Math.abs(
      salesEntradaPct +
        salesAnnualPct * 2 +
        salesChavesPct +
  ...
  return (
    <>
      <div className="print:hidden space-y-6 animate-fadeIn">
            ...
            <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4">
              <div>
                <div className="flex items-center gap-2 mb-1.5">
                  ...
                  <span className="text-zinc-500 font-mono text-[9px] uppercase font-bold">
                    Aprovação cicloCRED CAIXA instantânea
                  </span>
                </div>
                ...
                <p className="text-xs text-zinc-500 font-medium">
                  Calcule o enquadramento de regras, taxas e subsídios Minha
                  Casa Minha Vida juntamente com o plano de parcelamento
                  facilitado da entrada.
                </p>
              </div>
                ...
                {selectedLeadId && (
                  <div className="bg-emerald-50 border-2 border-emerald-500 p-2 text-left rounded-xl flex items-center gap-2 w-full xl:w-auto">
                    ...
                    <div className="min-w-0 pr-2">
                      <p className="text-[10px] font-extrabold text-emerald-950 leading-tight uppercase font-mono">
                        Lead Conectado
                      </p>
            ...
            {filteredLeadsForLink.length > 0 && (
              <div className="space-y-2">
                <div className="flex items-center justify-between text-[9px] font-mono font-black text-zinc-500 uppercase px-1">
                  <span>
                    Resultados de busca detectados (
                    {filteredLeadsForLink.length}):
                  </span>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  ...
                  <div className="flex flex-col gap-2 max-h-[180px] overflow-y-auto pr-1">
                    <span className="text-[10px] font-black text-zinc-500 border-b border-zinc-300/50 pb-1 mb-1 font-mono uppercase tracking-widest pl-1 sticky top-0 bg-zinc-50/80  z-10">
                      Recentes / Novos
                    </span>
                    {filteredLeadsForLink.filter((l) => l.status === "novo")
                      .length === 0 && (
                      <span className="text-[9px] text-zinc-400 font-bold p-1 italic select-none">
                        Lista Zerada do Sistema
                      </span>
                    )}
                    {filteredLeadsForLink
                  ...
                  {/* ATIVOS */}
                  <div className="flex flex-col gap-2 max-h-[180px] overflow-y-auto pr-1">
                    <span className="text-[10px] font-black text-indigo-500 border-b border-indigo-200/50 pb-1 mb-1 font-mono uppercase tracking-widest pl-1 sticky top-0 bg-zinc-50/80  z-10">
                      Ativos / Em Progresso
                    </span>
                    {filteredLeadsForLink.filter(
                      (l) =>
                        l.status !== "novo" &&
                        l.status !== "arquivado" &&
                        l.status !== "perdi" &&
                    ...
                    {filteredLeadsForLink
                      ...
                      .map((lead) => (
                        ...
                        >
                          ...
                          <span className="text-[7.5px] opacity-75 font-normal uppercase italic bg-zinc-200/50 text-zinc-600 px-1 rounded shrink-0">
                            {getStatusLabelSim(lead.status)}
                          </span>
                        </button>
                      ))}
                  </div>
                  ...
                  {/* ARQUIVADOS */}
                  <div className="flex flex-col gap-2 max-h-[180px] overflow-y-auto pr-1">
                    <span className="text-[10px] font-black text-rose-500 border-b border-rose-200/50 pb-1 mb-1 font-mono uppercase tracking-widest pl-1 sticky top-0 bg-zinc-50/80  z-10">
                      Arquivados / Perdidos
                    </span>
                    {filteredLeadsForLink.filter(
                      (l) =>
        ...
        {activeSim === "financing" && (
          <div className="w-full">
            <div className="bg-white border-4 border-zinc-950 p-6 lg:p-8 rounded-3xl shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] text-zinc-900 space-y-6">
              <div className="flex justify-between items-center border-b pb-3">
                <div>
                  <span className="text-[10px] uppercase font-mono font-black text-indigo-600 block leading-none">
                    Amortização Simulada Caixa
                  </span>
                  <h3 className="text-sm font-black uppercase tracking-tight text-zinc-950 mt-1">
                    Tabela de Resultados Habitação
                  </h3>
                </div>
              ...
              {/* NOVO BLOCO 2 COLUNAS: Formulário à esquerda, Parcelas à direita */}
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                {/* Inputs Section (4/12 width) */}
                <div className="bg-zinc-50 border-4 border-zinc-950 p-6 rounded-3xl space-y-4">
                  <div className="border-b pb-2 flex justify-between items-start">
                    <div>
                      <span className="text-[10px] font-mono font-black text-indigo-650 uppercase">
                        Formulário Habitacional
                      </span>
                      <h3 className="text-sm font-black text-zinc-950 uppercase tracking-tight">
                        Parâmetros de Renda Caixa
                      </h3>
                    </div>
                    <button
                      type="button"
                  ...
                  <div className="space-y-1">
                    <label className="block text-[10px] font-mono font-black text-zinc-650 uppercase">
                      Nome do Cliente (Ficha/Proposta)
                    </label>
                    <input
                      type="text"
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setSimulatedPropertyPrice(Number(e.target.value) || 0)
                        }
                  ...
                  <div className="space-y-1.5">
                    <label className="block text-[10px] font-mono font-black text-zinc-650 uppercase">
                      Regime de Comprovação de Renda
                    </label>
                  ...
                  {professionType === "CLT" ? (
                    <div className="space-y-1 animate-fadeIn">
                      ...
                      <p className="text-[9px] text-zinc-400 font-mono leading-none">
                        Demonstrada via holerites ou carteira de trabalho
                        oficial.
                      </p>
                    </div>
                  ) : (
                    <div className="space-y-1 animate-fadeIn">
                      ...
                      <p className="text-[9px] text-zinc-400 font-mono leading-none">
                        Comprovada por extratos consolidados dos últimos 3
                        meses.
                      </p>
                    </div>
                  )}
                  ...
                  <div className="p-3 bg-zinc-50 border-2 border-zinc-950 rounded-2xl space-y-3">
                    <div className="flex items-center justify-between border-b pb-1">
                      <span className="block font-mono font-black text-[9px] uppercase tracking-wider text-zinc-500">
                        Composição Familiar
                      </span>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      />
                      <label
                        htmlFor="civil-married"
                        className="cursor-pointer select-none text-xs font-extrabold text-zinc-900 leading-none"
                      >
                    ...
                    {isMarried && (
                      <div className="space-y-1 mt-2 p-2 bg-white border border-dashed border-zinc-300 rounded-xl animate-scaleIn">
                        <label className="block text-[9px] font-mono font-black text-indigo-900 uppercase">
                          Renda da Esposa/Cônjuge (R$)
                        </label>
                        <div className="relative">
                          <span className="absolute left-2.5 top-1.5 text-xs text-zinc-400 font-bold">
                            R$
                          </span>
                          <input
                            type="number"
                        ...
                        <span className="text-[8.5px] text-indigo-600 font-medium block mt-1">
                          ✓ Renda agregada automaticamente à composição
                          familiar.
                        </span>
                      </div>
                    )}
                    ...
                    <div className="space-y-1">
                      <label className="block text-[9.5px] font-mono font-black text-zinc-650 uppercase">
                        Quantidade de Dependentes
                      </label>
                      <select
                        value={dependentsQty}
                      ...
                      <p className="text-[8.5px] text-zinc-400 font-mono tracking-tight leading-none mt-1">
                        *Dependentes na ficha ativam bônus máximos de subsídio
                        MCMV.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="p-3 bg-indigo-950 text-indigo-100 rounded-2xl border-2 border-zinc-950 flex justify-between items-center shadow-sm">
                    <div>
                      <span className="block text-[8px] font-mono font-black uppercase tracking-wider text-indigo-300 leading-none">
                        Renda Bruta Coletiva
                      </span>
                      <strong className="block text-sm font-mono font-black text-white mt-1">
                        R$ {grossIncome.toLocaleString("pt-BR")}
                      </strong>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-3.5">
                    ...
                    <div className="space-y-1">
                      ...
                      <input
                        ...
                        value={applicantAge || ""}
                        onChange={(e) =>
                          setApplicantAge(Number(e.target.value) || 30)
                        }
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="p-3 bg-emerald-50 border-2 border-emerald-400 rounded-2xl text-xs font-semibold">
                      <div className="flex items-start gap-2">
                        ...
                        <div className="cursor-pointer select-none">
                          <label
                            htmlFor="has-fgts-reduz"
                            className="block text-[11px] font-extrabold text-emerald-950 leading-none"
                          >
                            Tem 3 anos de FGTS?
                          </label>
                    ...
                    <div className="p-3 bg-amber-50 border-2 border-amber-400 rounded-2xl text-xs font-semibold">
                      <div className="flex items-start gap-2">
                        ...
                        <div className="cursor-pointer select-none">
                          <label
                            htmlFor="has-interest-reducer"
                            className="block text-[11px] font-extrabold text-amber-950 leading-none"
                          >
                    ...
                    <div className="p-3 bg-indigo-50 border-2 border-indigo-400 rounded-2xl text-xs font-semibold">
                      <div className="flex items-start gap-2">
                        ...
                        <div className="cursor-pointer select-none">
                          <label
                            htmlFor="has-casa-paulista"
                            className="block text-[11px] font-extrabold text-indigo-950 leading-none"
                          >
                  ...
                  <div className="space-y-1 text-xs">
                    <label className="block text-[10px] font-mono font-black text-zinc-650 uppercase">
                      Sistema de Amortização Bancária
                    </label>
                ...
                <div className="bg-zinc-50 border-4 border-zinc-950 p-6 rounded-3xl space-y-4 relative overflow-hidden">
                  <div className="absolute right-0 bottom-0 text-[100px] font-black font-sans -mr-8 -mb-10 text-zinc-200/40 select-none pointer-events-none">
                    CAIXA
                  </div>

                  <div className="space-y-4 relative z-10">
                    
                    {/* The 4 Metrics visual cards block moved here */}
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      <div className="p-3 bg-indigo-50 border-2 border-indigo-400 rounded-2xl text-center space-y-0.5 shadow-sm">
                        <span className="block text-[9px] font-mono font-extrabold text-indigo-700 uppercase">
                          Enquadramento MCMV
                        </span>
                        <strong className="block text-xs font-black text-indigo-950 font-sans">
                          {mcmvBracketName}
                        </strong>
                      </div>

                      <div className="p-3 bg-zinc-55 border-2 border-zinc-950 rounded-2xl text-center space-y-0.5 shadow-sm">
                        <span className="block text-[9px] font-mono font-extrabold text-zinc-650 uppercase">
                          Subsídio Concedido
                        </span>
                        <strong className="block text-xs font-black text-zinc-900 font-mono tracking-tight">
                          R$ {calculatedSubsidy.toLocaleString("pt-BR")}
                        </strong>
                      </div>

                      <div className="p-3 bg-emerald-50 border-2 border-emerald-400 rounded-2xl text-center space-y-0.5 shadow-sm">
                        <span className="block text-[9px] font-mono font-extrabold text-emerald-700 uppercase">
                          Juros Nominal
                        </span>
                        <strong className="block text-xs font-black text-emerald-950 font-mono">
                          {applicableInterestRate}% a.a.
                        </strong>
                      </div>

                      <div className="p-3 bg-amber-50 border-2 border-amber-400 rounded-2xl text-center space-y-0.5 shadow-sm">
                        <span className="block text-[9px] font-mono font-extrabold text-amber-800 uppercase">
                          Amortização
                        </span>
                        <strong className="block text-xs font-black text-amber-950 font-sans">
                          {amortizationSystem} / {loanMonths}m
                        </strong>
                      </div>
                    </div>

                    <h4 className="text-[11px] font-mono font-black text-zinc-550 uppercase border-b pb-1 mt-4">
                      Inteligência Preditiva: Parcelas Mensais
                    </h4>

                    {/* Inputs for downpayment facilitation layout */}
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-3 relative z-10 text-xs">
                      {/* Parcela de Ato */}
                      <div className="space-y-1">
                        <label className="block text-[9px] font-mono font-black text-zinc-400 uppercase">
                          Ato (Sinal)
                        </label>
                        <div className="relative">
                          <span className="absolute left-2 top-1.5 text-[10px] text-zinc-500 font-bold">
                            R$
                          </span>
                          <input
                            type="number"
                      ...
                      {/* Parcela de Anuais (2x) */}
                      <div className="space-y-1">
                        <label className="block text-[9px] font-mono font-black text-zinc-400 uppercase">
                          Anuais (Valor de cada uma - 2x)
                        </label>
                        <div className="relative">
                          <span className="absolute left-2 top-1.5 text-[10px] text-zinc-500 font-bold">
                            R$
                      ...
                      <div className="space-y-1">
                        ...
                        <select
                          value={tempoObra}
                          onChange={(e) =>
                            setTempoObra(Number(e.target.value) || 36)
                          }
                    ...
                    <div className="bg-indigo-50/70 p-3 rounded-2xl border-2 border-indigo-200 space-y-2">
                      <span className="text-[10px] font-black text-indigo-900 block uppercase font-mono">
                        🏦 1. Parcela do Financiamento Bancário
                        (Pós-Obra/Quitação)
                      </span>

                      <div className="grid grid-cols-2 gap-2 text-[11px]">
                        <div className="bg-white p-2 rounded-xl border border-indigo-100 flex flex-col justify-center">
                          <span className="text-zinc-500 font-bold block text-[9px] uppercase">
                            Primeira Parcela (SAC)
                          </span>
                          <strong className="text-xs font-black font-mono text-zinc-950">
                            R$ {firstPayment.toLocaleString("pt-BR")} /mês
                          </strong>
                        </div>
                        <div className="bg-white p-2 rounded-xl border border-indigo-100 flex flex-col justify-center">
                          <span className="text-zinc-500 font-bold block text-[9px] uppercase">
                          ...
                          <strong className="text-xs font-black font-mono text-zinc-950">
                            R$ {lastPayment.toLocaleString("pt-BR")} /mês
                          </strong>
                        </div>
                      </div>
                      ...
                      <div className="bg-white p-2 rounded-xl border border-indigo-100 flex items-center justify-between text-[10px]">
                        <span className="text-zinc-500 font-bold">
                          Margem Máxima de Renda (30%):
                        </span>
                        <strong className="font-mono text-indigo-900 font-black">
                          R$ {maxMonthlyPayment.toLocaleString("pt-BR")}
                        </strong>
                      </div>
                      ...
                      {firstPayment > maxMonthlyPayment && (
                        <div className="flex items-start gap-1 pb-1 text-[9.5px] text-red-800 leading-tight">
                          ...
                          <span>
                            <strong>⚠️ Parcela excede 30% da renda:</strong>{" "}
                            Necessita{" "}
                            <strong className="text-indigo-800">
                              agregar renda
                            </strong>{" "}
                            com co-adquirentes ou apresentar fiador/avalista!
                          </span>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="space-y-3 relative z-10 mt-4">
                      <h4 className="text-[11px] font-mono font-black text-zinc-550 uppercase border-b pb-1">
                        Distribuição de Recursos Requeridos
                      </h4>

                      <div className="space-y-2">
                        <div className="flex justify-between items-center bg-white p-2.5 rounded-xl border-2 border-indigo-500">
                          <span className="text-[11.5px] font-black text-indigo-800 uppercase font-mono">
                            Aprovado pela Caixa:
                          </span>
                          <strong className="text-sm font-black font-mono text-indigo-950">
                            R$ {financedAmount.toLocaleString("pt-BR")}
                          </strong>
                        </div>
                        {/* 2nd block removed as requested */}
                      </div>

                      {simulatedPropertyPrice > 0 &&
                        requiredDownpayment / simulatedPropertyPrice > 0.18 && (
                          <div className="flex items-start gap-2 p-2.5 bg-amber-50 border-2 border-amber-500 rounded-xl text-[10px] text-zinc-900 leading-tight">
                            ...
                            <div>
                              <strong className="text-amber-850 block uppercase text-[8.5px] font-mono">
                                👥 ENTRADA REQUERIDA SUPERIOR A 18%
                              </strong>
                              <span>
                                A entrada de{" "}
                                {(
                                ...
                                % superou o limite recomendado de 18% (R${" "}
                                {(simulatedPropertyPrice * 0.18).toLocaleString(
                                  "pt-BR",
                                )}
                                ).{" "}
                                <strong className="text-amber-900">
                                  É preciso agregar renda, compor um
                                  co-adquirente ou um fiador
                                </strong>{" "}
                                para aumentar o financiamento e viabilizar a
                                entrada.
                              </span>
                            </div>
                          </div>
                      ...
                      <p className="text-[10px] text-zinc-400 leading-tight font-medium">
                        *A entrada requerida de R${" "}
                        {requiredDownpayment.toLocaleString("pt-BR")} será
                        calculada no plano facilitado abaixo.
                      </p>
                    </div>

                    {/* PLANO DE PARCELAMENTO FACILITADO COM A CONSTRUTORA */}
                    <div className="bg-zinc-900 text-white p-4 rounded-xl space-y-3 border-2 border-zinc-950 relative overflow-hidden scale-90 origin-top mt-4 -mb-10">
                      ...
                      <div className="border-b border-zinc-800 pb-2 relative z-10 flex justify-between items-center">
                        <div>
                          <span className="text-[8.5px] font-mono font-black text-emerald-400 uppercase tracking-widest block leading-none">
                            PLANO CORRETOR COMPLETO
                          </span>
                        ...
                        <div className="px-2 py-0.5 rounded text-[8.5px] font-mono font-black uppercase bg-emerald-500 text-zinc-950 tracking-wider">
                          Entrada: R${" "}
                          {requiredDownpayment.toLocaleString("pt-BR")}
                        </div>
                      </div>

                      {/* Auto dynamic calculations result band */}
                      {(() => {
                        const totalFacilitadoAvulso =
                          valorAto + 2 * valorAnual + valorChaves;
                        const diferencaRestante = Math.max(
                          0,
                          requiredDownpayment - totalFacilitadoAvulso,
                        );
                        const valorMensalObra =
                          tempoObra > 0 ? diferencaRestante / tempoObra : 0;
                        ...
                        return (
                          <div className="relative z-10 bg-zinc-950/80 border border-zinc-800 p-3 rounded-xl grid grid-cols-1 md:grid-cols-3 gap-3 items-center">
                            <div>
                              <span className="block text-[8px] font-mono font-black text-zinc-500 uppercase leading-none">
                                Total Sinal + Balões
                              </span>
                              <strong className="block text-xs font-sans font-black text-zinc-350 mt-1">
                                R${" "}
                                {totalFacilitadoAvulso.toLocaleString("pt-BR")}
                              </strong>
                            ...
                            <div className="border-t md:border-t-0 md:border-l md:border-r border-zinc-800 py-1.5 md:py-0 md:px-3">
                              <span className="block text-[8px] font-mono font-black text-amber-400 uppercase leading-none">
                                Diferença Restante
                              </span>
                            ...
                            <div className="bg-emerald-950/55 border border-emerald-800 p-2 text-center rounded-lg">
                              ...
                              <strong className="block text-sm font-mono font-black text-emerald-400 mt-1">
                                R${" "}
                                {Math.round(valorMensalObra).toLocaleString(
                                  "pt-BR",
                                )}{" "}
                                /mês
                              </strong>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div className="space-y-3.5 bg-white border-2 border-zinc-950 p-4 rounded-2xl">
                        <h4 className="text-xs font-black uppercase text-indigo-950 border-b pb-1 font-sans flex items-center gap-1.5 animate-fadeIn">
                          ...
                          Enquadramento Técnico (Modificadores)
                        </h4>
                        ...
                        <div className="space-y-2.5 text-xs">
                          ...
                          <div className="flex justify-between items-start border-b border-zinc-100 pb-1.5 animate-fadeIn">
                            <div>
                              ...
                              <span className="text-[10px] text-zinc-500 font-medium">
                                Comprovação ativa para liberação
                              </span>
                            </div>
                            ...
                            <div className="text-right">
                              ...
                              {isMarried && (
                                <span className="block text-[10.5px] text-zinc-650 font-bold mt-1">
                                  + R$ {spouseIncome.toLocaleString("pt-BR")}{" "}
                                  (Esposa)
                                </span>
                              )}
                            </div>
                          ...
                          <div className="flex justify-between items-start border-b border-zinc-100 pb-1.5 animate-fadeIn">
                            <div>
                              ...
                              <span className="text-[10px] text-zinc-500 font-medium">
                                Fator de bonificação de subsídio
                              </span>
                            </div>
                            <div className="text-right">
                          ...
                          <div className="flex justify-between items-start border-b border-zinc-100 pb-1.5 animate-fadeIn">
                            <div>
                              ...
                              <span className="text-[10px] text-zinc-500 font-medium">
                                Taxas de juros subsidiadas CEF
                              </span>
                            </div>
                            <div className="text-right">
                              <span
                          ...
                          <div className="p-2.5 bg-blue-50/60 rounded-xl border border-blue-200 animate-fadeIn">
                            <div className="flex justify-between items-center">
                              <span className="font-black text-blue-950 uppercase text-[9.5px]">
                                Enquadramento MCMV:
                              </span>
                              <span className="text-[10.5px] font-mono font-extrabold text-blue-800">
                                {mcmvBracketName}
                              </span>
                            </div>
                            <p className="text-[9px] text-zinc-500 mt-1">
                              Sua Renda familiar consolidada de{" "}
                              <strong>
                                R$ {grossIncome.toLocaleString("pt-BR")}
                              </strong>{" "}
                              e dependentes liberaram{" "}
                              <strong>
                                R$ {calculatedSubsidy.toLocaleString("pt-BR")}
                              </strong>{" "}
                              de subsídios habitacionais diretos Caixa
                              {hasCasaPaulista
                                ? ` (com fomento estadual Casa Paulista de R$ ${(grossIncome <= 4400 ? 13000 : 10000).toLocaleString("pt-BR")} integrado)`
                                : ""}
                              .
                            </p>
                          </div>
                      ...
                      <div className="space-y-3.5 bg-white border-2 border-zinc-950 p-4 rounded-2xl flex flex-col justify-between">
                        <div>
                          <h4 className="text-xs font-black uppercase text-indigo-950 border-b pb-1 font-sans flex items-center gap-1.5">
                            ...
                            Documentação Física Oficial
                          </h4>
                          <p className="text-[10px] text-zinc-550 mb-3 font-medium">
                            Marque os documentos confirmados e verificados para
                            compor o dossiê digital de financiamento e dar
                            entrada na aprovação Caixa:
                          </p>
                          ...
                          <div className="space-y-2.5">
                            ...
                            >
                              ...
                              <div className="text-xs select-none">
                                <span
                                  className={`font-black uppercase tracking-tight block ${docCpfCnh ? "text-emerald-950" : "text-zinc-800"}`}
                                >
                                  🪪 CPF / CNH
                                </span>
                            ...
                            >
                              ...
                              <div className="text-xs select-none">
                                <span
                                  className={`font-black uppercase tracking-tight block ${docEndereco ? "text-emerald-950" : "text-zinc-800"}`}
                                >
                                  🏠 COMPROVANTE DE ENDEREÇO
                                </span>
                                <span className="text-[10px] text-zinc-500 block">
                                  Contas de água, luz ou gás com prazo menor que
                                  90 dias
                                </span>
                              </div>
                            </label>
                            ...
                            >
                              ...
                              <div className="text-xs select-none">
                                <span
                                  className={`font-black uppercase tracking-tight block ${docRenda ? "text-emerald-950" : "text-zinc-800"}`}
                                >
                            ...
                            >
                              ...
                              <div className="text-xs select-none">
                                <span
                                  className={`font-black uppercase tracking-tight block ${docExtratos ? "text-emerald-950" : "text-zinc-800"}`}
                                >
                                  📊 EXTRATOS BANCÁRIOS
                                </span>
                                <span className="text-[10px] text-zinc-500 block">
                        ...
                        <div
                          className={`mt-4 p-2.5 border-2 rounded-xl text-center text-xs font-black font-mono flex items-center justify-center gap-2 ${
                            docCpfCnh && docEndereco && docRenda && docExtratos
                        ...
                        >
                          ...
                          docExtratos ? (
                            <>
                              ...
                              <span>
                                🗂️ DOSSIÊ COMPLETO E PRONTO PARA APROVAÇÃO
                                CAIXA!
                              </span>
                            </>
                          ) : (
              ...
              <div className="p-4 border-2 border-dashed border-zinc-200 rounded-2xl flex items-start gap-3">
                ...
                <div className="text-xs text-zinc-650 space-y-1">
                  <span className="font-extrabold uppercase text-zinc-950 font-sans block">
                    Sua simulação está qualificada em altíssimo nível!
                  </span>
                  <p>
                    O simulador habitacional estima os indexadores baseando-se
                    nas regras de concessão da Caixa Econômica Federal 2026. A
                    comprovação de renda e simulação podem sofrer alterações
                    após exame final de documentos cadastrados do cliente.
                  </p>
                </div>
              </div>
            </div>
        ...
        {activeSim === "sales" && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start animate-fadeIn">
            ...
            <div className="lg:col-span-4 bg-white border-4 border-zinc-950 p-6 rounded-3xl shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] space-y-5">
              ...
              <div className="space-y-1">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setSalesPropertyPrice(Number(e.target.value) || 0)
                    }
                    className="w-full bg-zinc-50 border-2 border-zinc-950 rounded-xl py-2 pl-9 pr-3 text-xs font-bold"
                  />
                </div>
              ...
              <div className="space-y-1">
                <label className="block text-[10px] font-mono font-black text-zinc-650 uppercase">
                  Lifespan de Obras (Meses de Disponível)
                </label>
                <input
                  type="number"
                  min={6}
                  ...
                  onChange={(e) =>
                    setSalesConstructionMonths(Number(e.target.value) || 36)
                  }
              ...
              <div className="space-y-4 pt-1">
                <span className="block font-mono font-black text-[9px] uppercase tracking-wider text-zinc-450 border-b pb-1">
                  Ajuste de Fluxo Percentual
                </span>
                ...
                <div className="space-y-1 bg-zinc-50 p-2.5 rounded-xl border">
                  ...
                  />
                  <span className="block text-[9.5px] text-zinc-400 font-mono text-right">
                    Parcela balão anual: R${" "}
                    {salesAnnualVal.toLocaleString("pt-BR")}
                  </span>
                </div>
                ...
                <div className="space-y-1 bg-zinc-50 p-2.5 rounded-xl border">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      const limitFinan =
                        100 -
                        (salesEntradaPct + salesAnnualPct * 2 + salesChavesPct);
                      setSalesFinanciamentoPct(
                        val > limitFinan ? limitFinan : val,
                      );
                    }}
                    className="w-full accent-blue-600 cursor-pointer"
                  />
                  <span className="block text-[9.5px] text-zinc-400 font-mono text-right">
                    Crédito após habite-se: R${" "}
                    {salesFinanciamentoVal.toLocaleString("pt-BR")}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="lg:col-span-8 bg-white border-4 border-zinc-950 p-6 rounded-3xl shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] text-zinc-900 space-y-5">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 border-b-2 pb-4">
                <div>
                  <span className="text-[10px] font-mono font-black text-indigo-650 block uppercase">
                    Resultados demonstrativos
                  </span>
                  <h3 className="text-sm font-black text-zinc-950 uppercase">
                    Tabela de Amortizações Período Obra
                  </h3>
                </div>
                <div className="flex flex-wrap gap-2 items-center">
                  ...
                  <div
                    className={`px-2 py-1 text-[9px] font-mono font-black uppercase rounded border ${
                      isSalesBalanced
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-1">
                <div className="p-3 bg-zinc-50 border rounded-xl text-center">
                  ...
                  <strong className="block text-xs font-mono font-black text-zinc-950">
                    R$ {salesPropertyPrice.toLocaleString("pt-BR")}
                  </strong>
                </div>

                <div className="p-3 bg-indigo-50 border rounded-xl text-center">
                  <span className="block text-[8px] font-mono text-indigo-700 uppercase font-black">
                    Entrada Recomendada
                  </span>
                  <strong className="block text-xs font-mono font-black text-indigo-950">
                    R$ {salesEntradaVal.toLocaleString("pt-BR")}
                  </strong>
                </div>

                <div className="p-3 bg-amber-50 border rounded-xl text-center">
                  <span className="block text-[8px] font-mono text-amber-800 uppercase font-black">
                    2 Balões Intermediários
                  </span>
                  <strong className="block text-xs font-mono font-black text-amber-950">
                    R$ {salesAnnualVal.toLocaleString("pt-BR")}
                  </strong>
                </div>

                <div className="p-3 bg-emerald-50 border rounded-xl text-center">
                  <span className="block text-[8px] font-mono text-emerald-800 uppercase font-black">
                    Mensais Obra ({salesConstructionMonths}x)
                  </span>
                  <strong className="block text-xs font-mono font-black text-emerald-950">
                    R$ {salesMonthlyWorkVal.toLocaleString("pt-BR")}
                  </strong>
                  <span className="text-[9px] text-zinc-400 font-mono">
                    ({salesMonthlyWorkPct.toFixed(1)}%)
                  </span>
                </div>
              </div>

              {salesPropertyPrice > 0 &&
                salesEntradaVal / salesPropertyPrice > 0.18 && (
                  <div className="flex items-start gap-2 p-2.5 bg-amber-50 border-2 border-amber-500 rounded-xl text-[10px] text-zinc-900 leading-tight">
                    <Users className="w-4 h-4 text-amber-700 shrink-0 mt-0.5" />
                    <div>
                      <strong className="text-amber-850 block uppercase text-[8.5px] font-mono">
                        👥 ENTRADA REQUERIDA SUPERIOR A 18% (PARCELA DE OBRA)
                      </strong>
                      <span>
                        A entrada sinal (ato) recomendada de R${" "}
                        {salesEntradaVal.toLocaleString("pt-BR")} (
                        {((salesEntradaVal / salesPropertyPrice) * 100).toFixed(
                          1,
                        )}
                        %) ultrapassou a recomendação confortável de 18% do
                        valor total do imóvel (R${" "}
                        {(salesPropertyPrice * 0.18).toLocaleString("pt-BR")}).{" "}
                        <strong className="text-indigo-800">
                          É preciso agregar renda, compor um co-adquirente ou um
                          fiador
                        </strong>{" "}
                        para otimizar os percentuais e viabilizar o plano de
                        parcelamento!
                      </span>
                    </div>
                  </div>
                  ...
                  <table className="w-full text-left text-xs border-collapse font-sans">
                    ...
                    <tbody className="divide-y divide-zinc-200">
                      ...
                      <tr className="bg-indigo-50/50 font-bold">
                        ...
                        <td className="p-2.5">
                          Sinal de Recursos Próprios (Assinatura)
                        </td>
                        <td className="p-2.5 text-right font-mono">
                          {salesEntradaPct}%
                        </td>
                        <td className="p-2.5 text-right font-mono pr-6">
                          R$ {salesEntradaVal.toLocaleString("pt-BR")}
                        </td>
                      </tr>
                      ...
                      {Array.from(
                        { length: salesConstructionMonths },
                        (_, idx) => {
                          const m = idx + 1;
                          const isAnnual =
                            m % 12 === 0 && m < salesConstructionMonths;
                          return (
                            <React.Fragment key={m}>
                              <tr className="hover:bg-zinc-100 transition-colors">
                                <td className="p-2 pl-4 font-mono text-zinc-500">
                                  Mês {m}
                                </td>
                                <td className="p-2 text-zinc-700">
                                  Mensalidade Período de Obras #{m}
                                </td>
                                ...
                                <td className="p-2 text-right font-mono text-zinc-950 pr-6">
                                  R${" "}
                                  {salesMonthlyWorkVal.toLocaleString("pt-BR")}
                                </td>
                              </tr>
                              {isAnnual && (
                                <tr className="bg-amber-50 font-black text-amber-950">
                                  ...
                                  <td className="p-2 text-amber-900">
                                    📣 Intermediária de Reforço Anual
                                  </td>
                                  <td className="p-2 text-right font-mono">
                                    {salesAnnualPct}%
                                  </td>
                                  <td className="p-2 text-right font-mono pr-6">
                                    R$ {salesAnnualVal.toLocaleString("pt-BR")}
                                  </td>
                                </tr>
                              )}
                      ...
                      <tr className="bg-emerald-50 font-black text-emerald-950">
                        <td className="p-2.5 pl-4 font-mono">
                          Mês {salesConstructionMonths}
                        </td>
                        <td className="p-2.5 text-emerald-900">
                          🔑 Saldo Final da Entrega das Chaves / Habite-se
                        </td>
                        <td className="p-2.5 text-right font-mono">
                          {salesChavesPct}%
                        </td>
                      ...
                      <tr className="bg-sky-50 text-sky-950 font-black border-t-2 border-zinc-900">
                        ...
                        <td className="p-3 text-right font-mono pr-6">
                          R$ {salesFinanciamentoVal.toLocaleString("pt-BR")}
                        </td>
                      </tr>
                    </tbody>
          ...
          <div className="bg-zinc-100 border-4 border-zinc-950 rounded-3xl shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] w-full max-w-4xl max-h-[92vh] flex flex-col overflow-hidden text-zinc-900">
            ...
            <div className="bg-zinc-950 text-white p-4 shrink-0 flex flex-wrap gap-3 items-center justify-between border-b-4 border-zinc-950">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h4 className="text-[10px] font-mono font-bold uppercase text-indigo-400">
                    Dossiê de Simulação de Crédito
                  </h4>
                  <strong className="text-sm font-black uppercase tracking-tight block">
                    Ficha de Proposta & Enquadramento Habitacional
                  </strong>
                </div>
              </div>
              <div className="flex items-center gap-2">
            ...
            <div className="bg-amber-50 text-amber-950 border-b-2 border-amber-200 px-6 py-2 text-[10px] font-bold font-mono uppercase flex items-center gap-1.5 shrink-0">
              💡 DICA: Configure o layout de margens como "Nenhum" e ative
              "Gráficos de plano de fundo" para alinhar perfeitamente na página
              A4.
            </div>
              ...
              <div className="bg-white text-zinc-900 p-6 font-sans shadow-lg w-full max-w-[21cm] border border-zinc-300 rounded-sm relative text-xs space-y-4">
                ...
                <div className="flex justify-between items-start border-b-2 border-zinc-900 pb-2">
                  <div>
                    ...
                    <p className="text-[8px] text-zinc-500 font-mono font-bold uppercase mt-0.5">
                      Estudo Comparativo Neutro — Base do Programa Federal MCMV,
                      Casa Paulista SP e Caixa Econômica Federal
                    </p>
                  </div>
                ...
                <div className="space-y-1">
                  <h4 className="text-[9px] font-black uppercase text-indigo-900 font-mono bg-zinc-100 px-2 py-0.5 rounded">
                    1. Perfil do Comprador e Composição de Renda
                  </h4>
                  <div className="grid grid-cols-3 gap-2 border border-zinc-200 p-2.5 rounded-lg bg-zinc-50/50 text-[10.5px]">
                    <div>
                      <span className="text-[7.5px] text-zinc-400 uppercase block font-semibold">
                        Cliente Final
                      </span>
                      <strong className="text-zinc-800 font-bold block truncate">
                        {clientCustomName || "Cliente Avulso"}
                      </strong>
                    </div>
                    <div>
                      <span className="text-[7.5px] text-zinc-400 uppercase block font-semibold">
                        Profissão / Origem Renda
                      </span>
                      <strong className="text-zinc-805 font-bold block">
                        {professionType === "CLT"
                          ? "💼 CLT (Holerite / CTPS)"
                          : "🚀 Autônomo (Extratos)"}
                      </strong>
                    ...
                    <div>
                      ...
                      <strong className="text-zinc-805 font-bold block font-mono">
                        R${" "}
                        {isMarried
                          ? spouseIncome.toLocaleString("pt-BR", {
                    ...
                    <div>
                      ...
                      <strong className="text-zinc-850 font-bold block">
                        {dependentsQty || "0"} dependente(s)
                      </strong>
                    </div>
                    <div>
                      <span className="text-[7.5px] text-zinc-400 uppercase block font-semibold">
                      ...
                      <strong className="text-zinc-805 font-bold block">
                        {hasFGTS
                          ? "✓ Sim (Reduz Juros de Balcão)"
                          : "Inativo / Menor"}
                      </strong>
                    </div>
                    <div>
                      <span className="text-[7.5px] text-zinc-400 uppercase block font-semibold">
                        Idade da Ficha (Seguros)
                      </span>
                      <strong className="text-zinc-805 font-bold block">
                        {applicantAge} anos
                      </strong>
                    ...
                    <div>
                      <span className="text-[7.5px] text-indigo-700 uppercase block font-black font-mono">
                        RENDA FAMILIAR BRUTA
                      </span>
                      <strong className="text-indigo-900 text-[11.5px] font-black block font-mono">
                        R${" "}
                        {grossIncome.toLocaleString("pt-BR", {
                          minimumFractionDigits: 2,
                        })}
                      </strong>
                    </div>
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex flex-wrap gap-2 p-2 border border-zinc-200 rounded-lg bg-zinc-50/20 text-[9px] tracking-tight">
                    <span
                      className={`px-2 py-1 rounded-md border flex items-center gap-1 font-bold ${docCpfCnh ? "bg-emerald-50 text-emerald-800 border-emerald-300" : "bg-zinc-100 text-zinc-500 border-zinc-200"}`}
                    >
                      <span>{docCpfCnh ? "✓" : "✗"}</span>
                      <span>Documento CNH/CPF</span>
                    </span>
                    <span
                      className={`px-2 py-1 rounded-md border flex items-center gap-1 font-bold ${docEndereco ? "bg-emerald-50 text-emerald-800 border-emerald-300" : "bg-zinc-100 text-zinc-500 border-zinc-200"}`}
                    >
                      <span>{docEndereco ? "✓" : "✗"}</span>
                      <span>Comprovante Endereço</span>
                    </span>
                    <span
                      className={`px-2 py-1 rounded-md border flex items-center gap-1 font-bold ${docRenda ? "bg-emerald-50 text-emerald-800 border-emerald-300" : "bg-zinc-100 text-zinc-500 border-zinc-200"}`}
                    >
                      <span>{docRenda ? "✓" : "✗"}</span>
                      <span>Comprovante Renda</span>
                    </span>
                    <span
                      className={`px-2 py-1 rounded-md border flex items-center gap-1 font-bold ${docExtratos ? "bg-emerald-50 text-emerald-800 border-emerald-300" : "bg-zinc-100 text-zinc-500 border-zinc-200"}`}
                    >
                ...
                <div className="space-y-1.5">
                  <h4 className="text-[9.5px] font-black uppercase text-indigo-900 font-mono bg-zinc-100 px-2 py-1 rounded">
                    3. Resultado de Crédito Caixa Econômica Federal
                  </h4>
                      ...
                      <table className="w-full text-left font-mono text-[10px]">
                        <tbody className="divide-y divide-zinc-200">
                          ...
                          <tr>
                            ...
                            <td className="p-2 text-zinc-800 font-bold">
                              R${" "}
                              {simulatedPropertyPrice.toLocaleString("pt-BR")}
                            </td>
                            <td className="p-2">Prazo Caixa Contratado</td>
                            <td className="p-2 text-zinc-805 font-bold">
                              {loanMonths} meses ({loanMonths / 12} anos)
                            </td>
                          </tr>
                          <tr>
                            <td className="p-2 pl-3 text-emerald-800 font-bold">
                              Subsídio MCMV Concedido
                            </td>
                            <td className="p-2 text-emerald-700 font-black">
                              R$ {calculatedSubsidy.toLocaleString("pt-BR")}
                            </td>
                            <td className="p-2 text-zinc-650">
                              Taxa Juros Nominal
                            </td>
                            <td className="p-2 font-black text-zinc-900">
                              {applicableInterestRate}% a.a.
                            </td>
                          </tr>
                          <tr className="bg-indigo-50 font-black">
                            <td className="p-2.5 pl-3 text-indigo-905 uppercase text-[9px]">
                              Aprovado Financiamento Caixa
                            </td>
                            <td className="p-2.5 text-indigo-900 text-xs">
                              R$ {financedAmount.toLocaleString("pt-BR")}
                            </td>
                            <td className="p-2.5 text-zinc-700 uppercase text-[9px]">
                              Entrada Necessária Própria
                            </td>
                            <td className="p-2.5 text-indigo-950 text-xs">
                              R$ {requiredDownpayment.toLocaleString("pt-BR")}
                            </td>
                          </tr>
                          <tr>
                            <td className="p-2 pl-3">
                              1ª Prestação Estimada Caixa
                            </td>
                            <td className="p-2 font-bold text-zinc-800">
                              R$ {firstPayment.toLocaleString("pt-BR")}
                            </td>
                            <td className="p-2">
                              Última Prestação Estimada Caixa
                            </td>
                            <td className="p-2 text-zinc-700">
                              R$ {lastPayment.toLocaleString("pt-BR")}
                            </td>
                ...
                <div className="space-y-1.5">
                  <h4 className="text-[9.5px] font-black uppercase text-indigo-900 font-mono bg-zinc-100 px-2 py-1 rounded">
                    4. Parcelamento de Entrada Facilitada com a Construtora
                  </h4>
                  <div className="overflow-x-auto border border-zinc-200 rounded-lg custom-scrollbar">
                    <div className="min-w-[600px] w-full">
                        ...
                        <thead>
                          <tr className="bg-zinc-100 font-bold border-b border-zinc-200 text-zinc-600">
                            ...
                            <th className="p-2 text-right pr-4">
                              Financeiro Estimado (R$)
                            </th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-zinc-200">
                          <tr>
                            <td className="p-2 pl-3 font-sans font-bold">
                              Ato / Sinal Próprio
                            </td>
                            <td className="p-2 text-zinc-650">
                              Ato irrevogável no fechamento de contrato
                            </td>
                            <td className="p-2 text-right pr-4 font-black text-zinc-900">
                              R$ {valorAto.toLocaleString("pt-BR")}
                            </td>
                          </tr>
                          <tr>
                            <td className="p-2 pl-3 font-sans font-semibold">
                              Balões Intermediários (2 parcelas)
                            </td>
                            <td className="p-2 text-zinc-650">
                              Balões anuais programados para 12 e 24 meses
                            </td>
                            <td className="p-2 text-right pr-4 text-zinc-800">
                              2x de R$ {valorAnual.toLocaleString("pt-BR")}
                              <span className="text-[9px] text-zinc-400 font-normal block font-sans">
                                Total de R${" "}
                                {(valorAnual * 2).toLocaleString("pt-BR")}
                              </span>
                            </td>
                          </tr>
                          <tr>
                            <td className="p-2 pl-3 font-sans font-semibold">
                              Fração de Chaves (Entrega)
                            </td>
                            <td className="p-2 text-zinc-650">
                              Devido no Habite-se físico da incorporadora
                            </td>
                            <td className="p-2 text-right pr-4 font-bold text-zinc-800">
                              R$ {valorChaves.toLocaleString("pt-BR")}
                            </td>
                          </tr>
                          {(() => {
                            const totalFac =
                              valorAto + 2 * valorAnual + valorChaves;
                            const difRestante = Math.max(
                              0,
                              requiredDownpayment - totalFac,
                            );
                            const valMensalObra =
                              tempoObra > 0 ? difRestante / tempoObra : 0;
                            return (
                              <>
                                <tr className="bg-zinc-50 font-bold">
                                  <td className="p-2 text-indigo-900 pl-3">
                                    Saldo Amortizável em Obras
                                  </td>
                                  <td className="p-2 text-zinc-500">
                                    Saldo residual da entrada estendido
                                  </td>
                                  <td className="p-2 text-right pr-4 text-indigo-900">
                                    R$ {difRestante.toLocaleString("pt-BR")}
                                  </td>
                                </tr>
                                <tr className="bg-indigo-50 font-black">
                                  <td className="p-2.5 pl-3 text-indigo-950 uppercase text-[9px] font-sans">
                                    🧱 Mensalidade Período Obra (Mensais)
                                  </td>
                                  <td className="p-2.5 text-right pr-4 text-indigo-950 text-xs">
                                    {tempoObra} parcelas de R${" "}
                                    {Math.round(valMensalObra).toLocaleString(
                                      "pt-BR",
                                    )}{" "}
                                    /mês
                                  </td>
      ...
      {createPortal(
        <div
          id="print-sheet-ciclocred"
          className="hidden print:block absolute top-0 left-0 text-black bg-white select-none pointer-events-none p-4 font-sans w-[21cm] min-h-[29.7cm] text-[10.5px] leading-tight space-y-3"
        >
          {/* Header */}
          <div className="flex justify-between items-start border-b-2 border-zinc-900 pb-2">
            <div>
              <h1 className="text-base font-black uppercase text-zinc-950">
                SIMULAÇÃO CRÉDITO HABITACIONAL OFICIAL
              </h1>
              <p className="text-[8px] text-zinc-500 font-mono font-bold uppercase">
                VIA DE ARQUIVO REGULAMENTAR • QUALIFICAÇÃO INTEGRANTE MCMV &
                FLUXO
              </p>
            </div>
            <div className="text-right text-[8px] font-mono font-bold text-zinc-650">
              <p>
                Ficha:{" "}
                <span className="text-zinc-950 font-black">
                  SIM-2026-{activeSim === "financing" ? "MCMV" : "INC"}-
                  {Math.floor(100000 + Math.random() * 900000)}
                </span>
              </p>
              <p>
          ...
          {/* 1. Profile */}
          <div className="space-y-1">
            <h4 className="text-[9px] font-bold uppercase bg-zinc-100 p-0.5 rounded font-mono text-indigo-950">
              1. Identificação Geral do Cliente e Proponente
            </h4>
            <div className="grid grid-cols-3 gap-1 p-1.5 border border-zinc-200 rounded text-[9px]">
              <div>
                Cliente: <strong>{clientCustomName || "Cliente Avulso"}</strong>
              </div>
              <div>
                Renda Declarada:{" "}
                <strong>
              ...
              <div>
                Tipo Profissional: <strong>{professionType}</strong>
              </div>
              <div>
                Estado Civil:{" "}
                <strong>{isMarried ? "Casado" : "Solteiro"}</strong>
              </div>
              <div>
                Renda Cônjuge:{" "}
                <strong>
              ...
              <div>
                Dependentes: <strong>{dependentsQty}</strong>
              </div>
              <div>
                Tempo FGTS (3+ anos):{" "}
                <strong>{hasFGTS ? "✓ Sim" : "Não"}</strong>
              </div>
              <div>
                Idade: <strong>{applicantAge} anos</strong>
              </div>
              <div>
                RENDA INTEGRADA:{" "}
                <strong>
          ...
          <div className="space-y-0.5">
            <h4 className="text-[9px] font-bold uppercase bg-zinc-100 p-0.5 rounded font-mono text-indigo-950">
              2. Documental de Dossiê Físico
            </h4>
            <table className="w-full text-left text-[8.5px] border border-zinc-200 rounded">
              <tbody className="divide-y divide-zinc-200">
                <tr>
                  <td className="p-1 pl-2 font-bold">
                    Documentos CPF e RG / CNH
                  </td>
                  <td className="p-1 text-right">
                    {docCpfCnh ? "[✓] ENQUADRADO" : "[ ] PENDENTE"}
                  </td>
                </tr>
                <tr>
                  <td className="p-1 pl-2 font-bold">
                  ...
                  <td className="p-1 text-right">
                    {docEndereco ? "[✓] ENQUADRADO" : "[ ] PENDENTE"}
                  </td>
                </tr>
                <tr>
                  <td className="p-1 pl-2 font-bold">
                    Comprovantes de renda oficiais (Holerite/CTPS)
                  </td>
                  <td className="p-1 text-right">
                    {docRenda ? "[✓] ENQUADRADO" : "[ ] PENDENTE"}
                  </td>
                </tr>
                <tr>
                  <td className="p-1 pl-2 font-bold">
                    Extratos bancários consolidados com carimbos
                  </td>
                  <td className="p-1 text-right">
                    {docExtratos ? "[✓] ENQUADRADO" : "[ ] PENDENTE"}
                  </td>
                </tr>
              </tbody>
            </table>
          ...
          <div className="space-y-0.5">
            <h4 className="text-[9px] font-bold uppercase bg-zinc-100 p-0.5 rounded font-mono text-indigo-950">
              3. Resultado Habitacional Caixa
            </h4>
            <table className="w-full text-left font-mono text-[8.5px] border border-zinc-200 rounded">
              <tbody className="divide-y divide-zinc-200">
                ...
                <tr>
                  <td className="p-1 pl-2 text-emerald-800">
                    Subsídio Caixa (Desconto)
                  </td>
                ...
                <tr className="bg-zinc-100 font-bold">
                  <td className="p-1.5 pl-2 text-indigo-900">
                    APROVADO CAIXA CEF
                  </td>
                  <td className="p-1.5 text-indigo-900 font-bold">
                    R$ {financedAmount.toLocaleString("pt-BR")}
                  </td>
                  <td className="p-1.5 text-zinc-900">ENTRADA REQUERIDA</td>
                  <td className="p-1.5 text-indigo-950 font-bold">
                    R$ {requiredDownpayment.toLocaleString("pt-BR")}
                  </td>
                </tr>
              </tbody>
          ...
          <div className="space-y-0.5">
            <h4 className="text-[9px] font-bold uppercase bg-zinc-100 p-0.5 rounded font-mono text-indigo-950">
              4. Cronograma de Entrada Facilitada (Construtora)
            </h4>
            <table className="w-full text-left font-mono text-[8.5px] border border-zinc-200 rounded">
              <tbody className="divide-y divide-zinc-200">
                <tr>
                ...
                <tr>
                  ...
                  <td className="p-1 text-right pr-2">
                    2x de R$ {valorAnual.toLocaleString("pt-BR")} (Total R${" "}
                    {(valorAnual * 2).toLocaleString("pt-BR")})
                  </td>
                </tr>
                <tr>
                ...
                {(() => {
                  ...
                  return (
                    <>
                      <tr className="bg-zinc-50">
                        <td className="p-1 pl-2 text-indigo-900">
                          Saldo Restante na Obra
                        </td>
                        <td className="p-1 text-right pr-2 text-indigo-900">
                          R$ {difRestante.toLocaleString("pt-BR")}
                        </td>
                      </tr>
                      <tr className="bg-indigo-50 font-black">
                        <td className="p-1.5 pl-2 text-indigo-955">
                          🧱 Mensalidade Período Obra
                        </td>
                        <td className="p-1.5 text-right pr-2 font-black text-indigo-900">
                          {tempoObra} parcelas mensais de R${" "}
                          {Math.round(valMensalObra).toLocaleString("pt-BR")}{" "}
                          /mês
                        </td>
                      </tr>
                    </>
